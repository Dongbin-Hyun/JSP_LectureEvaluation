...
<%
	...
	if (request.getParameter("lectureYear") != null) {
		try {
			lectureYear = Integer.parseInt(request.getParameter("lectureYear"));//정수형 변환
		} catch (Exception e){
			System.out.println("강의연도 데이터 오류 발생");
		}
	}
	...
	if (lectureName == null || professorName == null || lectureYear == 0 || semesterDivide == null || 
			lectureDivide == null || evaluationTitle.equals("") || evaluationContent.equals("") || totalScore == null ||
			creditScore == null || comfortableScore == null || lectureScore == null || evaluationTitle == null ||
	...
	if (result == -1) {
		PrintWriter script = response.getWriter();
		script.println("<script>");
		script.println("alert('글을 등록하지 못했습니다.');");
		script.println("history.back();");
		script.println("</script>");
		script.close();
